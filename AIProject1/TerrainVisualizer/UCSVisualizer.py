...
from UCS import show_route, perform_analysis
from Visualizer import Visualizer

class UCSVisualizer(Visualizer):

	def __init__(self):
		super(UCSVisualizer, self).__init__()

	def run_search(self):
		return show_route(self.map, self.start_pos, self.end_pos, self.get_heuristic('None'))
	...
	def write_search_analysis(self, runtime_list, pathlength_list, num_nodes_traversed_list, cost_incurred_list):
		...
		analysis_file_path = os.path.join(analysis_files_folder, "UCS_"+self.heuristic_name)
...
if __name__ == "__main__":
	...
	if option == '0':
		...
		visualizer.analyze(visualizer.get_heuristic('None'))
