...
from AStarWeighted import show_route, perform_analysis
from Visualizer import Visualizer

class ASWVisualizer(Visualizer):

	def __init__(self):
		super(ASWVisualizer, self).__init__()

	def run_search(self):
		return show_route(self.map, self.start_pos, self.end_pos, self.get_heuristic('manhattan distance'), self.get_weight())

	def run_search_analysis(self, runtime_list, pathlength_list, num_nodes_traversed_list, cost_incurred_list, heuristic_function):
		perform_analysis(self.map, self.start_pos, self.end_pos, heuristic_function, self.weight, runtime_list, pathlength_list, num_nodes_traversed_list, cost_incurred_list)
	...
	def write_search_analysis(self, runtime_list, pathlength_list, num_nodes_traversed_list, cost_incurred_list):
		...
		analysis_file_path = os.path.join(analysis_files_folder, "AStarWeighted_"+self.heuristic_name+"_"+str(self.weight))
