...
class node:
    ...
    def updatePriority(self, xStart, yStart, xDest, yDest):
        ...
        self.priority = self.distance + w*self.heuristic
    ...
    while len(pq[pqi]) > 0:
        ...
        for i in range(dirs):
            ...
            if not (xdx < 0 or xdx > n-1 or ydy < 0 or ydy > m - 1 or map[ydy][xdx][0] == 1 or closed_nodes_map[ydy][xdx] == 1 or map[ydy][xdx][0] == '0'): # if blocked cell don't go there
                ...
                m0 = node(xdx, ydy, n0.distance, n0.priority, map[ydy][xdx][0], (i + dirs / 2) % dirs, n0.parenttype, heuristic_func)
...
def show_route(map, start_pos, end_pos, heuristic_func, weight):
...
def perform_analysis(map, start_pos, end_pos, heuristic_function, weight, runtime_list, pathlength_list, num_nodes_traversed_list, cost_incurred_list):
