    ...
    while len(pq[pqi]) > 0:
        ...
        for i in range(dirs):
            ...
            if not (xdx < 0 or xdx > n-1 or ydy < 0 or ydy > m - 1 or the_map[ydy][xdx][0] == 1 or closed_nodes_map[ydy][xdx] == 1 or the_map[ydy][xdx][0] == '0'): # if blocked cell don't go there
                ...
                m0 = node(xdx, ydy, n0.distance, n0.priority, the_map[ydy][xdx][0], (i + dirs / 2) % dirs, n0.parenttype)
    ...
    if route == '':
        ...
        if len(route) > 0:
            ...
            for i in range(len(route)):
                ...
                x += dx[j]
                y += dy[j]
                the_map[y][x] = ('3', route[i].priority, route[i].distance, None)
            else:
